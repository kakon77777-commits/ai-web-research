...
def test_final_package_metadata_and_console_entry_are_frozen():
...
def test_offline_cli_version_api_and_doctor():
    ...
    assert version.returncode == 0 and '1.0.0' in version.stdout and 'public API 1.0' in version.stdout
    api = run_cli('api', '--json')
    assert api.returncode == 0 and json.loads(api.stdout)['public_api_version'] == '1.0'
    assert json.loads(api.stdout)['package_version'] == '1.0.0'
