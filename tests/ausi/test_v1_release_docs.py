...
DOCS = {
    ...
    'final': Path('docs/release/RELEASE_v1.0.0.md'),
}
...
def test_required_release_docs_exist_and_lock_core_boundaries():
    ...
    assert 'import omphalos' in text['quick'] and '1.0.0' in text['quick']
    ...
    assert '`1.0.0` is the first final package version' in text['final']
    assert 'synthetic' in text['final'].lower() and 'live Provider' in text['final']
