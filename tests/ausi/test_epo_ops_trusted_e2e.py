...
class FakeResponse:
    def __init__(self, text=OPS_XML):
        self.text = text
...
def build(tmp_path):
    ...
    adapters.register(EpoOpsAdapter())
...
def action():
    return SearchAction(
        action_id="epo-e2e-1",
        task_id="patent-task-1",
        epoch_id="patent-epoch-1",
        ...
        parameters={"classification": "G06F16/00", "scheme": "cpc", "range": "1-10"},
        guards=(),
...
async def test_epo_ops_runs_through_common_policy_trusted_asset_and_receipt_runtime(tmp_path):
    ...
    try:
        result = await trusted.execute(
            action(),
            ExecutionContext(
                task_id="patent-task-1",
                epoch_id="patent-epoch-1",
                registry_snapshot_id="patent-registry-1",
                services={
                    "epo_ops_http_client": FakeClient(FakeResponse(OPS_XML)),
        ...
        assert result.observation.artifacts[0].id == "epo:publication:EP1234567A1"
        assert len(result.materialized_assets) == 1

        materialized = result.materialized_assets[0]
        envelope = materialized.usage_envelope
        assert AcquisitionAction.AUTOMATED_QUERY in envelope.permissions
        assert AcquisitionAction.INTERNAL_USE in envelope.permissions
        assert AcquisitionAction.DISTRIBUTE_DERIVED in envelope.permissions
        assert AcquisitionAction.REDISTRIBUTE_RAW in envelope.prohibitions

        action_receipts = receipt_store.list_search_action_receipts("patent-epoch-1")
        assert len(action_receipts) == 1
        assert action_receipts[0].provider_ref.id == "provider.epo_ops"
        assert action_receipts[0].result_count == 1
        assert action_receipts[0].artifact_refs == ("epo:publication:EP1234567A1",)

        final = recorder.finalize(
            receipt_id="patent-receipt-1",
            task_id="patent-task-1",
            epoch_id="patent-epoch-1",
            registry_snapshot_id="patent-registry-1",
            planner_id="planner.rule",
            planner_version="0.1.0",
            stop_reason="FIRST_PATENT_PROVIDER_SLICE_COMPLETE",
            status=SearchReceiptStatus.PARTIAL,
            created_at="2026-08-31T12:00:02+00:00",
            metadata={"declared_scope": "EPO OPS bibliographic/classification"},
        )
